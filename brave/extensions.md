🧩 Brave Extensions

- **uBlock Origin** — ID: `cjpalhdlnbpafiamejdnhcphjbkeiagm`
- **React Developer Tools** — ID: `fmkadmapgofadopljbjfkapdkoienihi`
- **Vue.js devtools** — ID: `nhdogjmejiglipccpnnnanhbledajbpd`
- **Return YouTube Dislike** — ID: `gebbhagfogifgggkldgodflihgfeippi`
- **SponsorBlock for YouTube** — ID: `mnjggcdmjocbbbhaepdhchncahnbgone`
- **Save Image As PNG** — ID: `nkokmeaibnajheohncaamjggkanfbphi`
- **Shazam** — ID: `mmioliijnhnoblpgimnlajmefafdfilb`
- **Unhook** — ID: `khncfooichmfjbepaaaebmommgaepoid`
- **Dark Reader** — ID: `eimadpbcbfnmbkopoojfekhnkhdbieeh`
