...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    float distortion = gnoise(vec3(0., y * .01, t * 200.)) * (glitchAmount * 1.5 + .03);
    distortion *= gnoise(vec3(0., y * .02, t * 100.)) * (glitchAmount * .8 + .01);
    ...
    col += (.03 + .15 * glitchAmount) * (hash33(vec3(fragCoord, mod(float(iFrame),
					1000.))).r) * displayNoise;
    col -= (.08 + .2 * glitchAmount) * (sin(4. * t + uv.y * iResolution.y * 1.75))
					* displayNoise;
