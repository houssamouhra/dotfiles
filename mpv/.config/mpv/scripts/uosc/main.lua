...
config = {
  ...
  types = {
    ...
    media = comma_split(options.video_types .. ',' .. options.audio_types .. ',' .. options.image_types .. ',' .. options.playlist_types),
  ...
  chapter_ranges = (function()
    ...
    if options.chapter_ranges and options.chapter_ranges ~= '' then
      for _, definition in ipairs(split(options.chapter_ranges, ' *,+ *')) do
        ...
        if name and color and name:match '^[a-zA-Z0-9_]+$' and color:match '^[a-fA-F0-9]+$' and (#color == 6 or #color == 8) then
...
function update_duration()
  local duration = state._duration and ((state.rebase_start_time == false and state.start_time) and (state._duration + state.start_time) or state._duration)
...
function update_margins()
  ...
  if top == state.margin_top and bottom == state.margin_bottom and left == state.margin_left and right == state.margin_right then
    return
  end
...
mp.observe_property('demuxer-cache-state', 'native', function(prop, cache_state)
  ...
  if not (state.duration and (#cached_ranges > 0 or state.cache == 'yes' or (state.cache == 'auto' and state.is_stream))) then
  ...
  create_self_updating_menu_opener {
    ...
    serializer = function(playlist)
      ...
      for index, item in ipairs(playlist) do
        ...
        items[index] = {
          title = (not force_filename and title) and title or (is_protocol(item.filename) and item.filename or serialize_path(item.filename).basename),
          hint = tostring(index),
          active = item.current,
          value = index,
        }
    ...
    serializer = function(audio_device_list, current_device)
      ...
      for _, device in ipairs(audio_device_list) do
        if device.name == 'auto' or string.match(device.name, '^' .. ao) then
          ...
          items[#items + 1] = {
            title = device.description:sub(1, 7) == 'Default' and t('Default %s', device.description:sub(9)) or device.description,
            hint = hint,
