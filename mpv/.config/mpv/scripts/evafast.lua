...
local function adjust_speed()
  ...
  if speedup_target ~= nil then
    ...
    if current_time >= speedup_target then
      ...
      local time_for_correction =
        speed_transition(speed, math.max(math.min(options.speed_cap, options.subs_speed_cap and options.subs_speed_cap or options.speed_cap), 1.1)) -- not effective_speed_cap because it may lead to huge fluctuations in transition speed
...
local function evafast(keypress)
  ...
  if keypress['event'] == 'up' or keypress['event'] == 'press' then
    ...
    if
      ...
      and ((not options.subs_speed_cap or mp.get_property 'sub-start' == nil) and options.speed_cap or options.subs_speed_cap) ~= 1
    then
      speed_timer:kill()
      speed_timer = nil
      jumps_reset_speed = true
